...
// Bolt Optimization: Extract the sidebar list item into a React.memo component to prevent O(n) recalculations
// (specifically `formatDistanceToNow`) whenever the parent component state (like `searchTerm`) changes.
...
export function NotesDashboard() {
  ...
  const handleBackViewer = useCallback(() => setViewingNote(null), []);
  const handleEditViewer = useCallback(() => handleOpenModal(viewingNote), [handleOpenModal, viewingNote]);
  const handleDeleteViewerDialog = useCallback(() => setIsViewerDeleteDialogOpen(true), []);
            ...
            <div className="mt-8">
              ...
              <div className="hidden min-w-0 lg:block">
                <NoteViewer
                  ...
                  onEdit={handleEditViewer}
                  onCopy={handleCopyViewerNote}
                  onDelete={handleDeleteViewerDialog}
                />
              </div>
            </div>
          )}
